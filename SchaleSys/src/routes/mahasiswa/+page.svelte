						...
						{#if data.mahasiswa && data.mahasiswa.length > 0}
							{#each data.mahasiswa as m}
								...
								>
									<td class="p-6">
										<div
											class="w-12 h-12 bg-slate-300 dark:bg-slate-600 border-[3px] border-slate-900 dark:border-white flex items-center justify-center overflow-hidden"
										>
											{#if m.foto}
												<img
													src={m.foto}
										...
										{#if data.user?.role === "admin"}
											...
											>
												<a
													...
													class="text-slate-900 dark:text-white font-mono text-xs uppercase tracking-widest hover:text-primary transition-colors mr-4"
												>
													// EDIT
												</a>
												<form
