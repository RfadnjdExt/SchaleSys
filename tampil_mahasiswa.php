                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        try {
                            ...
                            $stmt_count = $koneksi->query($sql_count);
                            $row_count = $stmt_count->fetch(PDO::FETCH_ASSOC);
                            $total_pages = ceil($row_count['total'] / $limit);
    
                            // Ambil data halaman ini
                            // Menggunakan LIMIT offset, limit (standar SQL)
                            // Catatan: Postgres juga mendukung LIMIT limit OFFSET offset, tapi syntax MySQL 'LIMIT offset, limit' juga sering didukung atau kita ubah ke standar standar.
                            // Standar SQL: LIMIT limit OFFSET offset
                            $stmt = $koneksi->prepare("SELECT nim, nama_mahasiswa, fakultas, prodi, angkatan, foto FROM mahasiswa ORDER BY nim ASC LIMIT :limit OFFSET :offset");
